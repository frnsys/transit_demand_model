...
def process_transfers_mp():
    ...
    def update(arg):
        trip_id, txs = arg
        # transfers[trip_id] = dict(txs)
        print(txs)
        raise
        pbar.update()
    pool = Pool()
    for trip_id, trip_data in trips.items():
        pool.apply_async(process_transfers, args=(trip_id, trip_data), callback=update)
    pool.close()
    pool.join()
    pbar.close()
    return transfers
    ...
    for stop_id, arr, dep in trip_data['sched'][1:]:
        ...
        for trip_id_, sched in stops[stop_id].items():
            ...
            for start, (i,_,_,other_start) in txs.items():
                transfers[start][stop_id].append((trip_id_, base_transfer_time, i, other_start))
        # indirect transfers
...
def transfers_for_stop(trip_data, sched, time):
    ...
    for start in trip_data['starts']:
        ...
        try:
            ...
            idx = next(i for i, (idx, arr, dep, st) in enumerate(sched_) if dep >= start + time)
...
#         for stop_id, arr, dep in reversed(trip_data['sched']):
